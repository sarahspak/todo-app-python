...
def delete_todo(position):
    ...
    count = c.fetchone()[0]
    if count == 0:
        return "No tasks to delete! Try adding a task first."
    elif position >= count:
        return f"Task at {position} does not exist!"
    with conn:
...
def update_todo(position: int, task: Optional[str], category: Optional[str]):
    with conn:
